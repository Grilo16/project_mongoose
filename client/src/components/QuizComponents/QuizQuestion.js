...
const QuizQuestion = ({
  ...
  const checkAnswer = (e) => {
    ...
    if (submittedAnswer.toLowerCase() === eachAnswer) {
      ...
      const answerAlreadySubmitted = copyCorrectAnswers.find(
      ...
      if (!answerAlreadySubmitted) {
        ...
        setIsAnswerCorrect(
          'You have already completed this question, please try a new question!'
        )
      }
    } else {
  ...
  return (
    <>
      <li className='questionandanswer'>
        {eachQuestion}
        ...
        {isAnswerCorrect ? <p>{isAnswerCorrect}</p> : null}
      </li>
    </>
  )
}
